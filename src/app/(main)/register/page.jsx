...
{errors.name &&<p className='text-red-500 text-center font-bold'>{errors.name.message}</p>}
  <legend className="fieldset-legend font-bold text-black">Photo Url</legend>
  <input type="text" className="input rounded-full" 
   placeholder="Type Your photo Url"  {...register("photo",{required: "Photo Url field is required"})}/>
{errors.photo &&<p className='text-red-500 text-center font-bold'>{errors.photo.message}</p>}
